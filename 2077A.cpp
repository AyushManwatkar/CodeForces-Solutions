...
bool solve(vll& arr,ll tar,ll tsum){
    ll n=sz(arr);
    vector<vector<bool>> dp(tsum+1,vector<bool>(tar+1,false));
    dp[0][0]=true;
    fr(n){
        for(ll x=tsum;x>=arr[i];x--){
            for(ll count=tar;count>=1;count--){
                if(dp[x-arr[i]][count-1]) dp[x][count]=true;
            }
        }
    }
    return dp[tsum][tar];
...
int main() {
    ...
    while(t--){
        ...
        ll oddsum=a1;
        ll evesum=0;
        rep1(i,1,n) oddsum+=b[i];
        rep(i,n+1,2*n) evesum+=b[i];
        ll miss=oddsum-evesum;
        vll res;
        res.pb(a1);
        ll even_ptr=n+1,odd_ptr=1;
        
        rep(i,1,2*n+1){
            if(i%2==1){
                if(i==2*n-1) res.pb(miss);
                else res.pb(b[even_ptr++]);
            }else{
                res.pb(b[odd_ptr++]);
            }
        }
