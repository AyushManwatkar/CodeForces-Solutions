...
int main() {
    ...
    ll n,m; cin>>n>>m;
    // vll l(n),r(n),p(n),q(n);
    // fr(n) cin>>l[i]>>r[i]>>p[i]>>q[i];
    vector<vpll> seg(m+1);
    ll stpr=1;
    
    fr(n){
        ll l,r,p,q; cin>>l>>r>>p>>q;
        ll bspr=((q-p+mod)%mod)*modInverse(q)%mod;
        stpr=(stpr*bspr)%mod;
        ll odds=(1LL*p*modInverse((q-p+mod)%mod))%mod;
        seg[r].pb({l,odds});
    }
    vll dp(m+1,0);
    dp[0]=1;
    ...
    rep1(pos,1,m){
        for(auto& x:seg[pos]) {
            ll left=x.ff,odds=x.ss;
            dp[pos]=(dp[pos]+dp[left-1]*odds)%mod;
        }
    }
    ll ans=(stpr*dp[m])%mod;
    eprint(ans)
