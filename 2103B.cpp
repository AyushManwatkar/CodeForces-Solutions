...
int main() {
    ...
    while(t--){
        ...
        // Initialize counters - M will track segments
        int M = 1;  // Start with 1 to account for first segment
        int c01 = 0, c10 = 0;
        
        // Start from the second character
        for (int i = 1; i < n; ++i) {
            // Check for transitions between characters
            if (s[i] != s[i-1]) {
                M++;  // Found a new segment
                if (s[i-1] == '0') 
                    c01++;  // 0→1 transition
                else 
                    c10++;  // 1→0 transition
        ...
        // Calculate best possible reduction
        int best_gain = 0;
        if (c01 >= 2 || c10 >= 2)
            best_gain = 2;
        else if (M >= 2)
