...
int main() {
    ...
    ll t; cin>>t;
    while(t--){
        pll a,b,c; cin>>a.ff>>a.ss>>b.ff>>b.ss>>c.ff>>c.ss;
        if((a.ss==b.ss && c.ss<a.ss) || (c.ss==b.ss && a.ss<c.ss) || (a.ss==c.ss && b.ss<a.ss)){
            if(a.ss==b.ss) eprint(abs(a.ff-b.ff))
            else if(c.ss==b.ss) eprint(abs(c.ff-b.ff))
            else eprint(abs(c.ff-a.ff))
            continue;
        }
        eprint("0")
    }
