...
bool solve(ll n,ll s,ll m,vpll& tasks){
    sort(all(tasks));
    if(tasks[0].ff>=s) return true;
    rep(i,1,n){
        if(tasks[i].ff-tasks[i-1].ss>=s) return true;
    }
    if(m-tasks.back().ss>=s) return true;
...
int main() {
    ...
    while(t--){
        ll n,s,m; cin>>n>>s>>m;
        vpll tasks(n);
        fr(n) cin>>tasks[i].ff>>tasks[i].ss;
        if(solve(n,s,m,tasks)) eprint("YES")
        else eprint("NO")
    }
